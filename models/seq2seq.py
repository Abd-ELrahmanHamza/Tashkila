...
class Decoder(nn.Module):
    ...
    def __init__(
        ...
        self.embbeding = nn.Embedding(
            trg_vocab_size, embedding_dim, padding_idx=0)
        attention_in = embedding_dim
        if use_prenet:
            self.prenet = Prenet(embedding_dim, prenet_depth)
            attention_in = prenet_depth[-1]

        self.attention_layer = nn.GRUCell(
            encoder_dim + attention_in, attention_units)
        self.attention_wrapper = AttentionWrapper(
            attention_type, attention_units)
        ...
        self.decoder_rnns = nn.ModuleList(
            [nn.GRUCell(decoder_units, decoder_units)
    ...
    def decode(
        ...
        self.attention_hidden = self.attention_layer(
    ...
    def inference(self):
        ...
        diacritic = (
            torch.full((batch_size,), self.start_symbol_id).to(
                self.device).long()
        )
        outputs, alignments = [], []
        self.initialize()
    ...
    def initialize(self):
        ...
        self.decoder_hiddens = [
            Variable(torch.zeros(batch_size, self.decoder_units)).to(
                self.device)
        ...
        self.prev_alignment = Variable(
            torch.zeros(batch_size, src_len)).to(self.device)
